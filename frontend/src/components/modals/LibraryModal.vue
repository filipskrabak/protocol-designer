<template>
  ...
  <!-- Delete Confirmation Dialog (separate from main dialog) -->
...
const sortOptions = [
  ...
  //{ title: "Most fields", value: "fields_desc" },
];
...
const filteredProtocols = computed(() => {
  ...
  switch (sortBy.value) {
    ...
    case "author_asc":
      ...
      break;
    case "fields_desc":
      filtered.sort((a, b) => (b.fields?.length || 0) - (a.fields?.length || 0));
      break;
  }
...
// Load SVG previews when modal opens
watch(modalRef, async (isOpen) => {
  if (isOpen) {
